...
if __name__ == "__main__":
    ...
    num_depth = 48
    dataset = MVSDataset(r"D:\Datas\dtu_mini\dtu", '../lists/dtu/train.txt', 'train',
                         3, num_depth, interval_scale=1.06)

    dataloader = DataLoader(dataset, batch_size=1)
    item = next(iter(dataloader))

    imgs = item["imgs"][:, :, :, ::4, ::4]  # (B, N, 3, H/4, W/4)
    # imgs = item["imgs"]  # (B, N, 3, H, W)
    print(f"imgs.shape={imgs.shape}")
    # proj_matrices = item["proj_matrices"]   #(B, N, 2, 4, 4) dim=N: N view; dim=2: index 0 for extr, 1 for intric
    proj_matrices = item["proj_matrices"]["stage1"]
    ...
    featureNet = FeatureNet(base_channels=8, stride=4)
    features = []
    for nview_idx in range(imgs.size(1)):  # imgs shape (B, N, 3, H, W)
        img = imgs[:, nview_idx]
        features.append(featureNet(img))
    print(f'feature shape stage1: {features[0]["stage1"].shape}')  # feature shape (B, 4C, H/4, W/4)
    print(f'feature shape stage2: {features[0]["stage2"].shape}')  # feature shape (B, 2C, H/2, W/2)
    print(f'feature shape stage3: {features[0]["stage3"].shape}')  # feature shape (B, C, H, W)

    print("\n----------### step.2 depth infer")
    ndepths_stage = 48

    cur_depth = torch.randn(img.shape[0], img.shape[2]//4, img.shape[3]//4)

    depth_range_samples = get_depth_range_samples(cur_depth=cur_depth,
                                                  ndepth=ndepths_stage,
                                                  depth_inteval_pixel=2 * depth_interval,
                                                  dtype=img[0].dtype,
                                                  device=img[0].device,
                                                  shape=[img.shape[0], img.shape[2]//4, img.shape[3]//4],
                                                  max_depth=depth_max,
                                                  min_depth=depth_min)
    print(f'depth_range_samples.shape={depth_range_samples.shape}')


    cost_regularization = nn.ModuleList([CostRegNet(in_channels=featureNet.out_channels[i],base_channels=[8, 8, 8][i]) for i in range(3)])
    depthNet = cas_mvsnet.DepthNet()
    features_stage = [feat["stage1"] for feat in features]

    outputs_stage = depthNet(features_stage, proj_matrices,
                             depth_values=depth_range_samples,
                             num_depth=ndepths_stage,
                             cost_regularization=cost_regularization[0])
    depth = outputs_stage['depth']
    print(f'depth.shape: {depth.shape}')

    cur_depth = depth.detach()
    cur_depth = F.interpolate(cur_depth.unsqueeze(1), scale_factor=(2,2), mode='nearest')
    print(f'cur_depth.shape: {cur_depth.shape}')
    ...
    print("----------debug-------------------------")
    print(ref_img.shape)
    print(src_imgs[0].shape)
    print(ref_proj.shape)
    print(src_proj.shape)
    print("-----------end--------------------------")
